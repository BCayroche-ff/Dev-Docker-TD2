...
# Creer le secret Redis (requis par ArgoCD v3.x)
kubectl create secret generic argocd-redis --from-literal=auth="argocd-redis-secret" -n argocd
...
> **Note**: ArgoCD v3.x necessite un secret `argocd-redis` avec un mot de passe non-vide pour que Redis demarre correctement.
